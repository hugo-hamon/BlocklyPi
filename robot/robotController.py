...
def is_in_range(angle: int, angle_range: (int, int)):
    """
    @:param
...
def set_motor_position(motor_nb: int, angle: int):
    mi, ma = MOTORS_RANGES[motor_nb]
    in_range = is_in_range(angle, MOTORS_RANGES[motor_nb])
    if in_range == 0:
        servo_kit.servo[motor_nb].angle = angle
    elif in_range == -1:
        servo_kit.servo[motor_nb].angle = mi
    elif in_range == 1:
        servo_kit.servo[motor_nb].angle = ma


def shift_motor_position(motor_nb: int, angle: int):
    shifted_angle = servo_kit.servo[motor_nb].angle + angle
    mi, ma = MOTORS_RANGES[motor_nb]
    in_range = is_in_range(shifted_angle, MOTORS_RANGES[motor_nb])
    if in_range == 0:
        servo_kit.servo[motor_nb].angle += angle
    elif in_range == -1:
        servo_kit.servo[motor_nb].angle = mi
