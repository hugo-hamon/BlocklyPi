...
goog.DEPENDENCIES_ENABLED && (goog.included_ = {}, goog.dependencies_ = {
    ...
    goog.moduleLoaderState_ = null;
    for (f = 0; f < b.length; f++) if (e = b[f]) d.pathIsModule[e] ? goog.importModule_(goog.basePath + e) : goog.importScript_(goog.basePath + e); else throw goog.moduleLoaderState_ = g, Error("Undefined script input");
...
goog.retrieveAndExecModule_ = function (a) {
    if (!COMPILED) {
        ...
        e.send();
        d = e.responseText;
        if (null != d) e = goog.wrapModule_(a, d), goog.IS_OLD_IE_ ? (goog.dependencies_.deferred[b] = e, goog.queuedModules_.push(b)) : c(a, e); else throw Error("load of " + a + "failed");
    }
};
...
goog.string.compareVersions = function (a, b) {
    for (var c = 0, d = goog.string.trim(String(a)).split("."), e = goog.string.trim(String(b)).split("."), f = Math.max(d.length, e.length), g = 0; 0 == c && g < f; g++) {
        ...
        do {
            var m = l.exec(h) || ["", "", ""], q = p.exec(k) || ["", "", ""];
            if (0 == m[0].length && 0 == q[0].length) break;
            var c = 0 == m[1].length ? 0 : parseInt(m[1], 10), n = 0 == q[1].length ? 0 : parseInt(q[1], 10),
...
goog.events.BrowserEvent.prototype.init = function (a, b) {
    var c = this.type = a.type, d = null;
    if (c == goog.events.EventType.TOUCHSTART || c == goog.events.EventType.TOUCHMOVE) d = a.targetTouches[0]; else if (c == goog.events.EventType.TOUCHEND || c == goog.events.EventType.TOUCHCANCEL) d = a.changedTouches[0];
...
Blockly.Workspace.prototype.updateToolbox = function (a) {
...
Blockly.Bubble.prototype.renderArrow_ = function () {
    ...
    if (b == d && c == e) a.push("M " + b + "," + c); else {
        ...
        var k = Math.sin(h), l = Math.cos(h), p = this.getBubbleSize(),
            h = (p.width + p.height) / Blockly.Bubble.ARROW_THICKNESS, h = Math.min(h, p.width, p.height) / 2,
            p = 1 - Blockly.Bubble.ANCHOR_RADIUS / f, d = b +
                p * d, e = c + p * e, p = b + h * l, m = c + h * k, b = b - h * l, c = c - h * k,
...
Blockly.Connection.prototype.closest = function (a, b, c) {
    ...
    c = b = h;
    var k = null, l = this.sourceBlock_, p = this;
    if (e.length) {
        for (; 0 <= b && d(b);) b--;
...
Blockly.Trashcan.prototype.createDom = function () {
    this.svgGroup_ = Blockly.createSvgElement("g", {}, null);
    var a = Blockly.createSvgElement("clipPath", {id: "blocklyTrashBodyClipPath"}, this.svgGroup_);
    Blockly.createSvgElement("rect", {width: this.WIDTH_, height: this.BODY_HEIGHT_, y: this.LID_HEIGHT_}, a);
    ...
    a = Blockly.createSvgElement("clipPath", {id: "blocklyTrashLidClipPath"}, this.svgGroup_);
    Blockly.createSvgElement("rect", {width: this.WIDTH_, height: this.LID_HEIGHT_}, a);
...
Blockly.Xml.workspaceToDom = function (a) {
    ...
    for (var c = goog.dom.createDom("xml"), d = a.getTopBlocks(!0), e = 0, f; f = d[e]; e++) {
        ...
        g.setAttribute("x", a.RTL ? b - f.x : f.x);
        ...
        switch (h.nodeName.toLowerCase()) {
            ...
            case "comment":
                ...
                p && setTimeout(function () {
                    d.comment && d.comment.setVisible && d.comment.setVisible("true" == p)
                }, 1);
...
Blockly.WorkspaceSvg.prototype.paste = function (a) {
    if (this.rendered && !(a.getElementsByTagName("block").length >= this.remainingCapacity())) {
        ...
        if (!isNaN(c) && !isNaN(a)) {
            this.RTL && (c = -c);
            do for (var d = !1, e = this.getAllBlocks(), f = 0, g; g = e[f]; f++) g = g.getRelativeToSurfaceXY(), 1 >= Math.abs(c - g.x) && 1 >= Math.abs(a - g.y) && (c = this.RTL ? c - Blockly.SNAP_RADIUS : c + Blockly.SNAP_RADIUS, a += 2 * Blockly.SNAP_RADIUS,
                d = !0); while (d);
            b.moveBy(c, a)
        }
        b.select()
...
Blockly.Block.prototype.interpolateMsg = function (a, b) {
    ...
    for (var e = a.split(this.interpolateMsg.SPLIT_REGEX_), f = [], g = 0; g < e.length; g += 2) {
        ...
        if ((h = e[g + 1]) && "%" == h.charAt(0)) {
            var l = parseInt(h.substring(1), 10), p = arguments[l];
...
Blockly.BlockSvg.prototype.initSvg = function () {
    ...
    !this.eventsInit_ && Blockly.bindEvent_(this.workspace.getCanvas(), "blocklyWorkspaceChange", this, this.onchange);
    this.eventsInit_ = !0;
...
Blockly.BlockSvg.prototype.onMouseUp_ = function (a) {
    ...
    Blockly.doCommand(function () {
        Blockly.terminateDrag_();
        if (Blockly.selected && Blockly.highlightedConnection_) Blockly.localConnection_.connect(Blockly.highlightedConnection_), b.rendered && (Blockly.localConnection_.isSuperior() ? Blockly.highlightedConnection_ : Blockly.localConnection_).sourceBlock_.connectionUiEffect(), b.workspace.trashcan && b.workspace.trashcan.close(); else if (b.workspace.isDeleteArea(a) && Blockly.selected.isDeletable()) {
...
Blockly.BlockSvg.prototype.showContextMenu_ = function (a) {
    if (!this.workspace.options.readOnly && this.contextMenu) {
        ...
        if (this.isDeletable() && this.isMovable() && !b.isInFlyout) {
            ...
            if (!this.collapsed_) for (d = 0; d < this.inputList.length; d++) if (this.inputList[d].type == Blockly.INPUT_VALUE) {
                d = {enabled: !0};
            ...
            this.workspace.options.collapse && (this.collapsed_ ? (d = {enabled: !0}, d.text = Blockly.Msg.EXPAND_BLOCK, d.callback =
                function () {
                    b.setCollapsed(!1)
            ...
            var d = this.getDescendants().length, e = this.getNextBlock();
            e && (d -= e.getDescendants().length);
            d = {
                text: 1 == d ? Blockly.Msg.DELETE_BLOCK : Blockly.Msg.DELETE_X_BLOCKS.replace("%1",
                    String(d)), enabled: !0, callback: function () {
                    b.dispose(!0, !0)
                }
            };
            c.push(d)
...
Blockly.BlockSvg.prototype.renderCompute_ = function (a) {
    ...
    for (var d = 0, e = 0, f = !1, g = !1, h = !1, k = void 0, l = this.inputsInline && !this.isCollapsed(), p = 0, m; m = b[p]; p++) if (m.isVisible()) {
        var q;
        l && k && k != Blockly.NEXT_STATEMENT && m.type != Blockly.NEXT_STATEMENT ? q = c[c.length - 1] : (k = m.type, q = [], q.type = l && m.type != Blockly.NEXT_STATEMENT ?
            Blockly.BlockSvg.INLINE : m.type, q.height = 0, c.push(q));
        q.push(m);
...
Blockly.BlockSvg.prototype.renderDrawTop_ = function (a, b, c, d) {
    ...
    this.previousConnection && (a.push("H", Blockly.BlockSvg.NOTCH_WIDTH - 15), b.push("H", Blockly.BlockSvg.NOTCH_WIDTH - 15), a.push(Blockly.BlockSvg.NOTCH_PATH_LEFT),
        b.push(Blockly.BlockSvg.NOTCH_PATH_LEFT_HIGHLIGHT), this.previousConnection.moveTo(c.x + (this.RTL ? -Blockly.BlockSvg.NOTCH_WIDTH : Blockly.BlockSvg.NOTCH_WIDTH), c.y));
    a.push("H", d);
    b.push("H", d + (this.RTL ? -1 : 0));
    this.width = d
};
Blockly.BlockSvg.prototype.renderDrawRight_ = function (a, b, c, d, e, f, g) {
    for (var h, k = 0, l, p, m = 0, q; q = f[m]; m++) {
        ...
        if (this.isCollapsed()) {
            ...
            this.RTL ? b.push("l 8,0 0,3.8 7,3.2 m -14.5,9 l 8,4") : b.push("h 8");
            n = q.height - Blockly.BlockSvg.JAGGED_TEETH_HEIGHT;
            a.push("v", n);
            this.RTL && b.push("v", n - 2);
            this.width +=
            ...
            a.push("H", h);
            b.push("H", h + (this.RTL ? -1 : 0));
            a.push("v", q.height);
            this.RTL && b.push("v", q.height - 2)
        ...
        b.push("v", Blockly.BlockSvg.SEP_SPACE_Y - 1), k += Blockly.BlockSvg.SEP_SPACE_Y), l = k + Blockly.BlockSvg.FIELD_HEIGHT, n.align != Blockly.ALIGN_LEFT && (r = f.statementEdge - n.fieldWidth - 2 * Blockly.BlockSvg.SEP_SPACE_X, n.align == Blockly.ALIGN_RIGHT ? h += r : n.align == Blockly.ALIGN_CENTRE && (h += (r + h) / 2)), this.renderFields_(n.fieldRow, h, l), h = f.statementEdge + Blockly.BlockSvg.NOTCH_WIDTH, a.push("H", h), a.push(Blockly.BlockSvg.INNER_TOP_LEFT_CORNER), a.push("v", q.height - 2 * Blockly.BlockSvg.CORNER_RADIUS), a.push(Blockly.BlockSvg.INNER_BOTTOM_LEFT_CORNER),
            ...
            b.push("H", f.rightEdge)), l = e.x + (this.RTL ? -h : h + 1), p = e.y + k + 1, n.connection.moveTo(l, p), n.connection.targetConnection && (n.connection.tighten_(), this.width = Math.max(this.width, f.statementEdge + n.connection.targetBlock().getHeightWidth().width)), m == f.length - 1 || f[m + 1].type == Blockly.NEXT_STATEMENT) && (a.push("v", Blockly.BlockSvg.SEP_SPACE_Y), this.RTL && b.push("v", Blockly.BlockSvg.SEP_SPACE_Y - 1), k += Blockly.BlockSvg.SEP_SPACE_Y);
        k += q.height
    }
...
Blockly.BlockSvg.prototype.renderDrawBottom_ = function (a, b, c, d) {
    this.height = d + 1;
    this.nextConnection && (a.push("H", Blockly.BlockSvg.NOTCH_WIDTH + " " + Blockly.BlockSvg.NOTCH_PATH_RIGHT), this.nextConnection.moveTo(this.RTL ? c.x - Blockly.BlockSvg.NOTCH_WIDTH : c.x + Blockly.BlockSvg.NOTCH_WIDTH, c.y + d + 1), this.nextConnection.targetConnection && this.nextConnection.tighten_(), this.height += 4);
    this.squareBottomLeftCorner_ ? (a.push("H 0"), this.RTL || b.push("M", "1," + d)) : (a.push("H", Blockly.BlockSvg.CORNER_RADIUS), a.push("a",
...
Blockly.BlockSvg.prototype.renderDrawLeft_ = function (a, b, c, d) {
    this.outputConnection ? (this.outputConnection.moveTo(c.x, c.y), a.push("V", Blockly.BlockSvg.TAB_HEIGHT), a.push("c 0,-10 -" + Blockly.BlockSvg.TAB_WIDTH + ",8 -" + Blockly.BlockSvg.TAB_WIDTH + ",-7.5 s " + Blockly.BlockSvg.TAB_WIDTH + ",2.5 " + Blockly.BlockSvg.TAB_WIDTH + ",-7.5"), this.RTL ? (b.push("M", -.3 * Blockly.BlockSvg.TAB_WIDTH + ",8.9"), b.push("l", -.45 * Blockly.BlockSvg.TAB_WIDTH + ",-2.1")) : (b.push("V", Blockly.BlockSvg.TAB_HEIGHT - 1), b.push("m", -.92 * Blockly.BlockSvg.TAB_WIDTH +
        ",-1 q " + -.19 * Blockly.BlockSvg.TAB_WIDTH + ",-5.5 0,-11"), b.push("m", .92 * Blockly.BlockSvg.TAB_WIDTH + ",1 V 1 H 2")), this.width += Blockly.BlockSvg.TAB_WIDTH) : this.RTL || (this.squareTopLeftCorner_ ? b.push("V", 1) : b.push("V", Blockly.BlockSvg.CORNER_RADIUS));
    a.push("z")
};
// Copyright 2013 Google Inc.  Apache License 2.0
Blockly.Msg = {};
...
Blockly.Toolbox.prototype.init = function () {
    ...
    this.flyout_ = new Blockly.Flyout({parentWorkspace: a, RTL: a.RTL});
    ...
    function b(a, e) {
        for (var f = 0, g; g = a.childNodes[f]; f++) if (g.tagName) switch (g.tagName.toUpperCase()) {
            case "CATEGORY":
                ...
                h.blocks = [];
                "true" == g.getAttribute("expanded") && h.setExpanded(!0);
                e.add(h);
...
Blockly.Css.CONTENT = [".blocklySvg {", "  background-color: #fff;", "  outline: none;", "  overflow: hidden;", "}", ".blocklyWidgetDiv {", "  display: none;", "  position: absolute;", "  z-index: 999;", "}", ".blocklyTooltipDiv {", "  background-color: #ffffc7;", "  border: 1px solid #ddc;", "  box-shadow: 4px 4px 20px 1px rgba(0,0,0,.15);", "  color: #000;", "  display: none;", "  font-family: sans-serif;", "  font-size: 9pt;", "  opacity: 0.9;", "  padding: 2px;", "  position: absolute;", "  z-index: 1000;", "}", ".blocklyResizeSE {",
    ...
    "  white-space: nowrap;", "}", '.blocklyToolboxDiv[dir="RTL"] .blocklyTreeRow {', "  padding-right: 0;", "  padding-left: 1em !important;", "}", ".blocklyTreeRow:hover {", "  background-color: #e4e4e4;", "}", ".blocklyTreeSeparator {", "  border-bottom: solid #e5e5e5 1px;", "  height: 0px;", "  margin: 5px 0;", "}", ".blocklyTreeIcon {", "  background-image: url(<<<PATH>>>/sprites.png);", "  height: 16px;", "  vertical-align: middle;", "  width: 16px;", "}", ".blocklyTreeIconClosedLtr {", "  background-position: -32px -1px;",
    ...
    b.readOnly || b.hasScrollbars || c.addChangeListener(function () {
        if (0 == Blockly.dragMode_) {
            ...
            if (a.contentTop < f || a.contentTop + a.contentHeight > a.viewHeight + f || a.contentLeft < (b.RTL ?
                ...
                var l = k.getRelativeToSurfaceXY(), p = k.getHeightWidth(), m = f + 25 - p.height - l.y;
                0 < m && k.moveBy(0, m);
...
Blockly.init_ = function (a) {
    var b = a.options, c = a.options.svg;
    Blockly.bindEvent_(c, "contextmenu", null, function (a) {
        Blockly.isTargetInput_(a) || a.preventDefault()
    });
    Blockly.bindEvent_(c, "touchstart", null, function (a) {
        Blockly.longStart_(a, null)
    });
    Blockly.bindEvent_(window, "resize", null, function () {
        Blockly.svgResize(a)
    });
    Blockly.documentEventsBound_ || (Blockly.bindEvent_(document, "keydown", null, Blockly.onKeyDown_), Blockly.bindEvent_(document, "touchend", null, Blockly.longStop_), Blockly.bindEvent_(document, "touchcancel",
        null, Blockly.longStop_), document.addEventListener("mouseup", Blockly.onMouseUp_, !1), goog.userAgent.IPAD && Blockly.bindEvent_(window, "orientationchange", document, function () {
        Blockly.fireUiEvent(window, "resize")
    }), Blockly.documentEventsBound_ = !0);
    b.languageTree && (a.toolbox_ ? a.toolbox_.init(a) : a.flyout_ && (a.flyout_.init(a), a.flyout_.show(b.languageTree.childNodes), a.scrollX = a.flyout_.width_, b.RTL && (a.scrollX *= -1), c = "translate(" + a.scrollX + ", 0)", a.getCanvas().setAttribute("transform", c), a.getBubbleCanvas().setAttribute("transform",
        c)));
    ...
    if (b.hasSounds) {
        ...
        d.push(Blockly.bindEvent_(document, "touchstart", null,
            b))
    }
};
Blockly.updateToolbox = function (a, b) {
...
Blockly.longStart_ = function (a, b) {
    ...
    Blockly.longPid_ = setTimeout(function () {
        a.button = 2;
        if (b) b.onMouseDown_(a); else Blockly.onMouseDown_(a)
    }, Blockly.LONGPRESS)
};
